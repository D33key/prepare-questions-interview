...
export const questions: QuestionWithoutId[] = [
	...
	{
		title: 'Что такое сервис-воркеры?',
		answer:
			'**Сервис-воркеры (Service Workers)** - это скрипты, которые работают в фоновом режиме в браузере и обеспечивают дополнительные функциональные возможности, которые не доступны обычным веб-страницам.<br/><b/>Например некоторые возможности, как кэширование ресурсов(файлы) или работа в оффлайн-режиме. По сути, сайт может работать офлайн благодаря этому.',
	},
	{
		title: 'Какие есть операторы в JS?',
		answer:
			'1) Операторы присваивания (=)<br/>2) Операторы сравнения<br/>3) Арифметические операторы<br/> 4) Логические операторы ( ||, &&, ! ) <br/> 5) Условный (тернарный) оператор (условие? значение1 : значение2)<br/> 6) Унарные операторы принимают только один операнд. Например let y = -x  <br/> 7) Бинарные операторы принимают два операнда и выполняют операцию между ними.',
	},
	{
		title: 'Как метод sort сортирует массив? Что за параметры он принимает?',
		answer:
			'**Гибридный алгоритм**: комбинация **сортировки вставками** (Insertion Sort) и **слиянием** (Merge Sort).<br/>В метод sort мы передаем колбек, который принимает 2 параметра a & b, делает между ними вычисление и возвращает результат выражения. Варианты: <br/> 1) положительное число, а должен быть расположен после b  <br/> 2) отрицательное число, а должен быть расположен перед b<br/> 3) и ноль, если порядок не имеет значения.<br/><br/> Если колбек не предоставлен, элементы сортируются в лексикографическом порядке как строки Unicode.',
	},
	{
		title: 'Расскажите про чистые и нечистые функции?',
		answer:
			'**Чистая функция** - Возвращает результат, основанный только на своих аргументах, без зависимости от внешних переменных или состояния программы.  <br/>Не имеет побочных эффектов, то есть не изменяет состояние программы или какие-либо внешние переменные<br/><br/>**Нечистая функция** - Изменяет состояние программы или какие-либо внешние переменные.  Может иметь побочные эффекты, такие как вывод в консоль, изменение DOM, отправка HTTP-запросов и т. д.',
	},
	{
		title: 'Что такое делегирование событий?',
		answer:
			'Это техника обработки событий, при которой обработчик событий устанавливается на родительский элемент. Вместо установки обработчиков на каждый отдельный элемент, можно установить один обработчик на родительский элемент, что уменьшает нагрузку на браузер.',
	},
	{
		title: 'Что такое Конкатенация и углификация файлов?',
		answer:
			'Процесс оптимизации и сборки JavaScript-файлов, который обычно осуществляется с помощью инструментов сборки.<br/> **Конкатенация** - это процесс объединения нескольких файлов в один. Это позволяет уменьшить количество запросов к серверу и улучшить скорость загрузки страницы. <br/> **Углификация** - это процесс сжатия и минимизации размера файлов, удаляя все ненужные символы и пробелы из исходного кода',
	},
	{
		title: 'Map и SET что это?',
		answer:
			'Это объекты со своей спецификой. **Map** – это коллекция ключ-значение, где каждый ключ может иметь только одно значение. Map позволяет использовать ключи любого типа. <br/> **Set** – это коллекция уникальных значений любого типа, которая хранит только уникальные элементы. Элементы в Set уникальны, то есть в Set не могут быть повторяющиеся элементы.',
	},
	{
		title: 'Какая разница между spred и rest операторами?',
		answer:
			'**Spread**<br/> 1) используется для распаковки элементов из массивов или объектов <br/> 2) для создания копии объекта или объединения нескольких объектов в один. <br/> 3)  позволяет развернуть массив, чтобы каждый его элемент был отдельным аргументом в функции<br/> Пример: <br/> `const result = sum(...numbers.slice(0, 3))`<br/><br/> **Rest** <br/> 1)используется для сбора оставшихся аргументов.<br/>2) Rest позволяет передавать неограниченное количество аргументов функции в виде массива<br/> Пример<br/> `function sum(...args) {}`',
	},
	{
		title: 'Что такое intersection observer в JS?',
		answer:
			'**Intersection Observer** в JavaScript — это API, который позволяет асинхронно отслеживать пересечение элементов (или их появление/исчезновение) в области видимости (viewport) или внутри другого элемента-контейнера. <br/> Основные возможности: <br/> 1) Определяет, виден ли элемент на экране. <br/> 2) Отслеживает частичное или полное пересечение. <br/> 3) Работает эффективно (без постоянных вычислений в scroll или resize событиях). <br/> 4) Полезен для: <br/> - Ленивой загрузки изображений (lazy-load). <br/> - Бесконечной подгрузки контента (infinite scroll). <br/> - Анимаций при появлении элемента. <br/> - Сбора аналитики (например, когда пользователь доскроллил до определённого места).',
	},
	{
		title: 'Что такое имутабельность?',
		answer:
			'**Имутабельность** — это принцип программирования, при котором данные **не могут быть изменены** после их создания. Вместо изменения существующих данных создаются **новые копии** с нужными изменениями.',
	},
	{
		title: 'Что такое генераторы в JS?',
		answer:
			'**Генераторы** — это специальные функции, которые можно **приостанавливать** и **возобновлять**, сохраняя контекст выполнения между вызовами. Они позволяют генерировать последовательности значений "по требованию" (ленивые вычисления). `function* generateSequence() { yield 1; yield 2; return 3; }` <br/><br/> Зачем нужны? <br/> 1) **Ленивые вычисления** (Значения вычисляются только при запросе, что экономит память) <br/> 2) **Работа с асинхронным кодом** (Раньше использовались для реализации **async/await** (до появления нативных Promise) <br/> 3) **Итераторы** (Генераторы автоматически реализуют протокол **итератора**, упрощая создание перебираемых объектов) <br/> 4) **Сложные алгоритмы** (Например, обход деревьев или генерация уникальных ID)',
	},
	{
		title: 'Что такое лексическое окружение?',
		answer:
			'Концепция, которая связана с областью видимости переменных во время выполнения кода. Лексическое окружение определяет, как переменные и функции связаны между собой внутри кода и каким образом они доступны друг другу. <br/> Лексическое окружение состоит из двух основных компонентов: <br/> 1) Environment Record (запись окружения) - отвечает за хранение переменных и функций внутри определенного контекста выполнения кода (например, функции или блока кода). <br/> 2) Outer lexical environment reference (Ссылка на внешнее лексическое окружение) - это ссылка на лексическое окружение, в котором данная функция была определена. Другими словами, это ссылка на окружение, в котором произошло объявление данной функции',
	},
	{
		title: 'Как проверить 2 объекта на идентичность?',
		answer:
			'1) Использовать JSON.stringify для приведения объектов в строку и сравнить их уже как 2 строки <br/> 2) Сравнение через цикл for...in <br/> 3) Использование библиотеки lodash или underscore',
	},
	{
		title: 'Как сделать копию объекта?',
		answer:
			'1) С помощью оператора расширения объекта (spread operator) <br/> 2) С помощью метода Object.assign()  <br/> 3) С помощью метода JSON.parse() и JSON.stringify() <br/> 4) Метод StructuredClone (глубокая копия) НО, поддерживается не везде (есть ограничения по копированию, например, функции нельзя копировать) <br/> 5) Пройтись циклом',
	},
	{
		title: 'Какие перебирающие методы массивов вы знаете?',
		answer:
			'1) forEach – для перебора массива.  <br/> 2) filter – для фильтрации массива.  <br/> 3) every/some – для проверки массива. <br/> 4) map – для трансформации массива в массив. <br/> 5) reduce/reduceRight – для прохода по массиву с вычислением значения. <br/> 6) sort - сортирует массив',
	},
	{
		title: 'Как объединить массивы?',
		answer:
			'1) Можно использовать метод concat() <br/> 2) Можно использовать spread оператор',
	},
	{
		title: 'Как узнать находится ли элемент в массиве?',
		answer:
			'1) Использовать метод includes, который возвращает true если элемент находиться в массиве. <br/> 2) также можно использовать метод find, который возвращает найденный элемент, или возвращает undefined',
	},
	{
		title: 'Что такое контекст?',
		answer:
			'В JavaScript контекст (или контекст выполнения, execution context) — это окружение, в котором выполняется код. Он определяет, какие переменные, функции и объекты доступны в данный момент, а также значение ключевого слова this.',
	},
	{
		title: 'Что такое this?',
		answer:
			'*this* — это ссылка на объект, который является текущим контекстом выполнения. Контекстом является объект который в данный момент выполняет или вызывает функцию. Для стрелочной функции - это объект в котором она создана, а в обычной функции - которым она вызвана.',
	},
	{
		title: 'Как можно подменить контекст вызова функции?',
		answer:
			'Есть 3 метода: call, apply, bind. <br/> call и apply вызывают функцию c заданным контекстом. bind возвращает новую функцию с уже навсегда привязанным контекстом.',
	},
	{
		title: 'Можно ли изменить контекст фукнции которую возвратил метод bind?',
		answer: 'Нет, bind привязывает контекст навсегда.',
	},
	{
		title: 'Можно ли изменить контекст у стрелочной фукнции?',
		answer: 'Нет. Стрелочная функция не имеет методов call, apply, bind',
	},
	{
		title: 'Отличия стрелочной функции от обычной?',
		answer:
			'1) this <br/> - Обычная функция: свой this, зависит от вызова <br/> - Стрелочная: берёт this из внешней области. <br/> 2) arguments <br/> - Обычная: есть псевдомассив arguments. <br/> - Стрелочная: нет arguments, только ...rest <br/> 3) Конструктор (new) <br/> - Обычная: можно вызывать с new <br/> - Стрелочная: нельзя, вызовет ошибку. <br/> 4) super (в классах) <br/> - Обычная: имеет доступ к super. <br/> - Стрелочная: нет своего super. <br/> 5) Поднятие (Hoisting) <br/> - Обычная: можно вызвать до объявления. <br/> - Стрелочная: нельзя (ReferenceError).',
	},
	{
		title: 'Чем отличается функция конструктор и класс?',
		answer:
			'Класс - это синтаксический сахар над функцией конструктором. При создании экземпляра класса, методы описанные в нем попадают в прототип, а методы описанные внутри функции конструктора попадут в сам экземпляр.',
	}
];
